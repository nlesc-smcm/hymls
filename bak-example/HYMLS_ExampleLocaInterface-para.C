...
//  int row = 0; //count the row
 ...
 /**************** ///  Below is the rhs of Interacting Turing System ************/
           if (var==0)
          {
            vals[p]=-D_*sigma_*(dim_*2)*ind+alpha_-alpha_*r1_*x[ijk2]*x[ijk2]-r2_*x[ijk2]; cols[p++]=(gidx_t)ijk;
          ...
          {       
            vals[p]=-sigma_*(dim_*2)*ind+beta_+alpha_*r1_*x[ijk2]*x[ijk]+r2_*x[ijk2]; cols[p++]=(gidx_t)ijk;
...
/**************** ///  Below is the rhs of FitzHugh–Nagumo equation ************/
        ...
        //  add the constant term k for FitzHugh–Nagumo equation
          // if( ijk%2==0)
            // F[(gidx_t)ijk] = F[(gidx_t)ijk] - r2_;

      // F[(gidx_t)ijk] = F[(gidx_t)ijk] + Eigen[(gidx_t)ijk]*10;
...
{ 
  ...
  double vals[2*dim_+2];
   ...
   for (int i=0; i<currentJac_->NumMyRows(); i++)
  {
        gidx_t gid = currentJac_->GRID(i);
        ind2sub(nx_,ny_,nz_,dof_,gid,&i,&j,&k,&var);
...
/**************** ///  Below is the rhs of Interacting Turing System ************/
          if (var==0)
          {
            vals[p]=-D_*sigma_*(2*dim_)*ind+alpha_-alpha_*r1_*x[ijk2]*x[ijk2]-r2_*x[ijk2]; cols[p++]=(gidx_t)ijk;
          ...
          {
            vals[p]=-sigma_*(2*dim_)*ind+beta_+2*alpha_*r1_*x[ijk2]*x[ijk]+r2_*x[ijk2]; cols[p++]=(gidx_t)ijk;
            vals[p]=-alpha_+alpha_*r1_*x[ijk]*x[ijk]+r2_*x[ijk]; cols[p++]=(gidx_t)ijk2;
            vals[p]=sigma_*indl; cols[p++]=(gidx_t)im1jk;
            vals[p]=sigma_*indr; cols[p++]=(gidx_t)ip1jk;
            vals[p]=sigma_*ind; cols[p++]=(gidx_t)ijm1k;
            vals[p]=sigma_*ind; cols[p++]=(gidx_t)ijp1k;
...
/**************** ///  Below is the rhs of FitzHugh–Nagumo equation ************/
         /*if (var==0)
          ...
          {
            CHECK_ZERO(currentJac_->InsertGlobalValues((gid,2*dim_+2,vals,cols));
          }
          else
          {
            CHECK_ZERO(currentJac_->ReplaceGlobalValues((gid,2*dim_+2,vals,cols));
          }
        }     
