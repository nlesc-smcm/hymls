  ...
  lsParams_(lsParams)
{
  ...
  colMap_=HYMLS::MatrixUtils::CreateMap(nx_,ny_,nz_,dof_, 0, *comm_); 
 
  // finally make the Import/Export objects (transfer function
  // between the two maps)
  as2std = Teuchos::rcp(new Epetra_Import(*colMap_,*rowMap_));
  ...
  nullSpace_=Teuchos::rcp(new Epetra_MultiVector(*rowMap_,2));  
 ...
 // HYMLS::MatrixUtils::Random(*currentState_);//range is [-1,1]
  ...
  for (int i=0; i<nullSpace_->MyLength();i=i+2)
  {
    (*nullSpace_)[1][i]=1.0;
...
*/
  ...
  return pVector_;
    ...
    int exampleLocaInterface::Assembly2Solve
          (const Epetra_DistObject& source, Epetra_DistObject& target) const
        {
//        DEBUG("Enter Assembly2Standard");
#ifdef TESTING
        if (!(source.Map().SameAs(*colMap_)&&target.Map().SameAs(*rowMap_)))
          {
...
//        DEBUG("Leave Assembly2Standard");
      ...
      //
    int exampleLocaInterface::Solve2Assembly
        (const Epetra_DistObject& source, Epetra_DistObject& target) const
        {
//        DEBUG("Enter Standard2Assembly");
#ifdef TESTING
        if (!(source.Map().SameAs(*rowMap_)&&target.Map().SameAs(*colMap_)))
          {
          HYMLS::Tools::Error("Invalid Transfer Function called!",__FILE__,__LINE__);
          }
#endif
...
//  int row = 0; //count the row
 ...
 /**************** ///  Below is the rhs of Interacting Turing System ************/
           if (var==0)
          {
            cout << "D=" << D_ << "IND=" << ind << "r1=" << r1_<<"r2="<<r2_<<"x[ijk2]=" << x[ijk2]<< "x[ijk]="<<x[ijk]<<endl;
            vals[p]=-D_*sigma_*2*dim_*ind+alpha_-alpha_*r1_*x[ijk2]*x[ijk2]-r2_*x[ijk2]; cols[p++]=(gidx_other_t)ijk;
          ...
          {       
            vals[p]=-sigma_*2*dim_*ind+beta_+alpha_*r1_*x[ijk2]*x[ijk]+r2_*x[ijk2]; cols[p++]=(gidx_other_t)ijk;
...
/**************** ///  Below is the rhs of Interacting Turing System ************/
          ...
          {
            vals[p]=-sigma_*2*dim_*ind+beta_+2*alpha_*r1_*x[ijk2]*x[ijk]+r2_*x[ijk2]; cols[p++]=(gidx_other_t)ijk;
...
/**************** ///  Below is the rhs of FitzHugh–Nagumo equation ************/
         /*if (var==0)
          ...
          //set the phase condition by fixing one point
...
//      force_backup_=false;
  ...
  //solver->setNullSpace(nullSpace_);
// Teuchos::RCP<Epetra_CrsMatrix> mass = getMassMatrix();
 ...
 if (reuseBasis && eigenVectors_ != Teuchos::null)
    ...
    for (int i = 0; i < v0->MyLength(); i++)
      {
      if (v0->Map().GID(i) % dof_ == 1) //In Turing system, dof_=2, so instead of using if (v0->Map().GID(i) % dof_ == dim_)
        {
        (*v0)[0][i] = 0.0;
        }
      }
