...
AugmentedMatrix::AugmentedMatrix(Teuchos::RCP<Epetra_RowMatrix> A,
  ...
  if (!V->Map().SameAs(A->RowMatrixRowMap()) || !W->Map().SameAs(A->RowMatrixRowMap()))
    {
  ...
  if (!rowMapA.SameAs(A_->OperatorDomainMap()) || !rowMapA.SameAs(A_->OperatorRangeMap()))
    {
  ...
  for (int i = 0; i < Wloc_->MyLength(); i++)
    {
    if (!colMapA.MyGID(Wloc_->Map().GID64(i)))
...
*/
double AugmentedMatrix::NormInf() const
  {
  return -1.0;
  }
...
*/
double AugmentedMatrix::NormOne() const
  {
  return -1.0;
  }
...
bool AugmentedMatrix::HasNormInf() const
  {
  return false;
  }
