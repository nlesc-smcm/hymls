  ...
  int *iflag)
{
  ...
  *iflag = 0;
  ...
  if (!(map.SameAs(map0)))
  {
    ...
    {
    CHECK_ZERO(solver->setProjectionVectors(Teuchos::rcp<const Epetra_MultiVector>(&vec0, false), Teuchos::rcp<const Epetra_MultiVector>(&vec1, false)));
    }
  }
  else
  {
    ...
    {
    CHECK_ZERO(solver->setProjectionVectors(Teuchos::rcp<const Epetra_MultiVector>(Q_ptr, false), Teuchos::rcp<const Epetra_MultiVector>(BQ_ptr, false)));
    }
  }
