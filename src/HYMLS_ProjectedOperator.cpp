...
const Epetra_Comm & ProjectedOperator::Comm() const
  {
  if (useVorth_)
    return A_->Comm();
  else
...
const Epetra_Map & ProjectedOperator::OperatorDomainMap() const
  {
  if (useVorth_)
...
const Epetra_Map & ProjectedOperator::OperatorRangeMap() const
  {
  if (useVorth_)
