...
#ifndef ANASAZI_JACOBI_DAVIDSON_HPP
#define ANASAZI_JACOBI_DAVIDSON_HPP

/*! \file AnasaziJacobiDavidson.hpp
...
/*!
 * \brief Structure to contain pointers to JacobiDavidson state variables.
 */
template <class ScalarType, class MV>
struct JacobiDavidsonState {
    ...
    JacobiDavidsonState() : curDim(0), V(Teuchos::null), AV(Teuchos::null),
...
class JacobiDavidson : public Eigensolver<ScalarType,MV,OP>
{
  private:
    ...
    typedef PreconditionerTraits<ScalarType,MV,PREC>         PRECT;
  ...
  public:
     ...
     *  \brief Constructor.
     *
     * JacobiDavidson constructor with eigenproblem, parameters, and
     * solver utilities.
     *
     ...
     */
    void initialize( JacobiDavidsonState<ScalarType,MV> state );
     ...
     *
     * JacobiDavidson doesn't compute Ritz residual norms
     ...
     */
    JacobiDavidsonState<ScalarType,MV> getState();
        ...
        Teuchos::ParameterList                     &pl )
{
    ...
    TEUCHOS_TEST_FOR_EXCEPTION( d_testSpace!="V" && d_testSpace!="AV" && d_testSpace!="BV", std::invalid_argument,
        "Anasazi::JacobiDavidson: Test Space must be V, AV, or BV" );
    TEUCHOS_TEST_FOR_EXCEPTION( d_testSpace=="V" ? false : !d_haveB, std::invalid_argument,
        "Anasazi::JacobiDavidson: Test Space must be V for standard eigenvalue problem" );
...
void JacobiDavidson<ScalarType,MV,OP,PREC>::iterate()
{
    ...
    if( !d_initialized )
    {
        d_outputMan->stream(Warnings) << "WARNING: JacobiDavidson::iterate called without first calling initialize" << std::endl;
...
void JacobiDavidson<ScalarType,MV,OP,PREC>::setSize(int blockSize, int maxSubDim )
{
    if( blockSize != d_blockSize || maxSubDim != d_maxSubspaceDim )
    {
        ...
        d_outputMan->stream(Debug) << " >> Anasazi::JacobiDavidson: Allocating eigenproblem"
...
state )
{
    ...
    d_outputMan->stream(Debug) << " >> Anasazi::JacobiDavidson: Initializing state"
    ...
    if( reset_V )
    {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION( rank < d_blockSize, std::logic_error,
            "Anasazi::JacobiDavidson::initialize(): Error generating initial orthonormal basis" );
    }

    if( d_outputMan->isVerbosity(Debug) )
    {
        d_outputMan->stream(Debug) << " >> Anasazi::JacobiDavidson: Error in V^T V == I: "
...
void JacobiDavidson<ScalarType,MV,OP,PREC>::sortProblem( int numWanted )
{
    ...
    if( d_haveB )
    {
        ...
        std::stringstream ss;
        ss << "Anasazi::JacobiDavidson: TGSEN returned error code " << info << std::endl;
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
        info < 0, std::runtime_error, "Anasazi::JacobiDavidson::"
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
        info == 1, std::runtime_error, "Anasazi::JacobiDavidson::"
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
        info > 1, std::logic_error, "Anasazi::JacobiDavidson::"
        "sortProblem: LAPACK routine TRSEN returned error code INFO = "
...
void JacobiDavidson<ScalarType,MV,OP,PREC>::expandSearchSpace()
{
    ...
    TEUCHOS_TEST_FOR_EXCEPTION( rank != d_expansionSize, std::runtime_error,
        "Anasazi::JacobiDavidson::ExpandSearchSpace(): Orthonormalization of new vectors failed" );
...
void JacobiDavidson<ScalarType,MV,OP,PREC>::solveProjectedEigenproblem()
{
    if( d_haveB )
    {
        ...
        std::stringstream ss;
        ss << "Anasazi::JacobiDavidson: GGES returned error code " << info << std::endl;
        TEUCHOS_TEST_FOR_EXCEPTION( info!=0, std::runtime_error, ss.str() );
    }
    else
    {
        ...
        std::stringstream ss;
        ss << "Anasazi::JacobiDavidson: GEES returned error code " << info << std::endl;
        TEUCHOS_TEST_FOR_EXCEPTION( info!=0, std::runtime_error, ss.str() );
    }
}
                                             ...
                                             int N)
{
    ...
    TEUCHOS_TEST_FOR_EXCEPTION( (int) realParts.size()<N, std::runtime_error,
        "Anasazi::JacobiDavidson::SortValues: Number of requested sorted values greater than vector length." );
        ...
        Teuchos::SerialDenseMatrix<int,ScalarType> &X )
{
    ...
    if( d_haveB )
    {
        ...
        std::stringstream ss;
        ss << "Anasazi::JacobiDavidson: TGEVC returned error code " << info << std::endl;
        TEUCHOS_TEST_FOR_EXCEPTION( info!=0, std::runtime_error, ss.str() );
    }
    else
    {
        ...
        std::stringstream ss;
        ss << "Anasazi::JacobiDavidson: TREVC returned error code " << info << std::endl;
        TEUCHOS_TEST_FOR_EXCEPTION( info!=0, std::runtime_error, ss.str() );
    }
}
              ...
              Teuchos::SerialDenseMatrix<int,ScalarType> &X_beta )
{
    ...
    TEUCHOS_TEST_FOR_EXCEPTION( Nr>d_curDim, std::logic_error,
        "Anasazi::JacobiDavidson::ScaleEigenvectors: Matrix size exceeds current dimension");
    TEUCHOS_TEST_FOR_EXCEPTION( Nc>d_curDim, std::logic_error,
        "Anasazi::JacobiDavidson::ScaleEigenvectors: Matrix size exceeds current dimension");
    TEUCHOS_TEST_FOR_EXCEPTION( X_alpha.numRows()!=Nr, std::logic_error,
        "Anasazi::JacobiDavidson::ScaleEigenvectors: number of rows in Xalpha does not match X");
    TEUCHOS_TEST_FOR_EXCEPTION( X_alpha.numCols()!=Nc, std::logic_error,
        "Anasazi::JacobiDavidson::ScaleEigenvectors: number of cols in Xalpha does not match X");
    TEUCHOS_TEST_FOR_EXCEPTION( X_beta.numRows()!=Nr, std::logic_error,
        "Anasazi::JacobiDavidson::ScaleEigenvectors: number of rows in Xbeta does not match X");
    TEUCHOS_TEST_FOR_EXCEPTION( X_beta.numCols()!=Nc, std::logic_error,
        "Anasazi::JacobiDavidson::ScaleEigenvectors: number of cols in Xbeta does not match X");
    ...
    astream << "JacobiDavidson::ScaleEigenvectors: multiply returned error code " << err;
    ...
    bstream << "JacobiDavidson::ScaleEigenvectors: multiply returned error code " << err;
...
#endif // ANASAZI_JACOBI_DAVIDSON_HPP
