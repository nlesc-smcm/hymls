...
#ifndef ANASAZI_JACOBI_DAVIDSON_SOLMGR_HPP
#define ANASAZI_JACOBI_DAVIDSON_SOLMGR_HPP
...
class JacobiDavidsonSolMgr : public SolverManager<ScalarType,MV,OP>
{
    public:
    ...
    private:
        ...
        RCP< JacobiDavidson<ScalarType,MV,OP,PREC> >    d_solver;
        RCP< PREC >                                     d_prec;
...
class JacobiDavidsonSolMgr<std::complex<MagnitudeType>,MV,OP,PREC>
{
  public:

    typedef std::complex<MagnitudeType> ScalarType;
    JacobiDavidsonSolMgr(
...
//---------------------------------------------------------------------------//
template <class ScalarType, class MV, class OP, class PREC>
JacobiDavidsonSolMgr<ScalarType,MV,OP,PREC>::JacobiDavidsonSolMgr(
        const RCP<Eigenproblem<ScalarType,MV,OP> > &problem,
        const RCP<PREC> &prec,
        Teuchos::ParameterList &pl )
   : d_problem(problem), d_prec(prec)
{
    ...
    TEUCHOS_TEST_FOR_EXCEPTION( ortho!="DGKS" && ortho!= "SVQB" && ortho!="ICGS", std::invalid_argument,
                                "Anasazi::JacobiDavidsonSolMgr::constructor: Invalid orthogonalization type" );
    ...
    // Build solver
    d_outputMan->stream(Debug) << " >> Anasazi::JacobiDavidsonSolMgr: Building solver" << std::endl;
    d_solver = Teuchos::rcp( new JacobiDavidson<ScalarType,MV,OP,PREC>
        ( problem, d_prec, d_sortMan, d_outputMan, d_tester, d_orthoMan, pl ) );
}
...
//---------------------------------------------------------------------------//
template <class ScalarType, class MV, class OP, class PREC>
ReturnType JacobiDavidsonSolMgr<ScalarType,MV,OP,PREC>::solve()
{
    ...
    while( 1 )
    {
        ...
        JacobiDavidsonState<ScalarType,MV> state = d_solver->getState();
    ...
    if( sol.numVecs > 0 )
    {
        ...
        {
            d_outputMan->stream(Debug) << " >> Anasazi::JacobiDavidsonSolMgr: "
...
//---------------------------------------------------------------------------//
template <class ScalarType, class MV, class OP, class PREC>
void JacobiDavidsonSolMgr<ScalarType,MV,OP,PREC>::getRestartState(
        JacobiDavidsonState<ScalarType,MV> &state )
{
    TEUCHOS_TEST_FOR_EXCEPTION( state.curDim <= d_restartDim, std::runtime_error,
            "Anasazi::JacobiDavidsonSolMgr: State dimension at restart is smaller than Restart Dimension" );
    ...
    d_outputMan->stream(Debug) << " >> Anasazi::JacobiDavidsonSolMgr: Restarting with "
    ...
    if( d_outputMan->isVerbosity(Debug) )
    {
        ...
        std::stringstream os;
        os << " >> Anasazi::JacobiDavidsonSolMgr: Error in V^T V == I after restart : " << orthErr << std::endl;
    ...
    TEUCHOS_TEST_FOR_EXCEPTION( err!=0, std::runtime_error, "JacobiDavidsonSolMgr::getRestartState: multiply returned nonzero error code" );
...
#endif // ANASAZI_JACOBI_DAVIDSON_SOLMGR_HPP
